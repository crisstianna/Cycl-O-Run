...
<form class="connexion" name="loginform" id="loginform" action="<?= get_bloginfo('url') . '/wp/wp-login.php'; ?>" method="post">
            ...
            if (strpos($url, 'login/?user=empty')!==false) {
                echo "<div class='login_failed' style=\"color:red;font-weight:bold;\">Veuillez renseigner l'adresse email</div>";
            }
            if (strpos($url, 'login/?pwd=empty')!==false) {
                echo "<div class='login_failed' style=\"color:red;font-weight:bold;\">Veuillez renseigner le mot de passe</div>";
            }
            if (strpos($url, 'login/?login=empty')!==false) {
                echo "<div class='login_failed' style=\"color:red;font-weight:bold;\">Les champs \"email\" et \"mot de passe\" sont vides</div>";
            }
            if (strpos($url,'login/?login=failed') !== false) {
                echo "<div class='login_failed' style=\"color:red;font-weight:bold;\">Email et/ou mot de passe incorrect(s)</div>";
            } 
            ?>
            ...
            <div class="connexion__page__inputs">
                <div class="login-username connexion__email">
                    <label class="connexion__label" for="user_login">Email</label>
                ...
                <div class="login-remember">
                    ...
                    <a href="<?= get_bloginfo('url') . '/wp/wp-login.php?action=lostpassword'?>">Mot de passe oublié</a>
                </div>
            </div>
